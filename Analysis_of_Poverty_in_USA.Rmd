...
```
##Executive Summary
This document presents an analysis of poverty rate among counties in the United States. The analysis is based on data compiled from a wide range of sources, mainly the U.S. Census Bureau’s Annual Social and Economic Supplement (ASEC) and the Current Population Survey (CPS), which are made publicly available by the United States Department of Agriculture Economic Research Service (USDA ERS). 
...
Let's look at the descriptive statistics of the response variable. Notice the median is below the mean for about 1 percent, which might indicate that there are some counties with poverty rate unusually high. The distribution of poverty rate more explicitly shows the right-skewness.
```{r, echo=FALSE}
...
```{r, echo=FALSE}
temp <- TrainingData %>% mutate(type = factor(gsub("_(.+)", "", area__ubif_sim))) %>% group_by(type) %>% summarize(count=n(), median=round(median(poverty_rate),3), mean=round(mean(poverty_rate),3), var=round(var(poverty_rate),3))

library(knitr)
kable(temp)

#par(mfrow=c(2,1))#don't work with ggplot
...
TrainingData %>% mutate(type = factor(gsub("_(.+)", "", area__ubif_sim))) %>% filter(type != "Metropolitan") %>% ggplot(aes(x=poverty_rate, fill=type)) + geom_density(bw=3, alpha=0.5)
```
...
```
The distribution of the difference looks like a bell-shaped curve, centered at zero. The blue vertical lines indicate the 2.5% and 97.5% quantile respectively. As in the graph above, the red vertical line lies within the 95% zone, which means no significant evidence that micropolitan is different from non-metro.So to reduce dimension, I would first consider classifying counties as metro or non-metro in the modeling process later on, instead of metropolitan, micropolitan and noncore. But there are still other factors hidden in area__rucc and area__urban_influence, namely population and adjacency.

####Population
(5) Let's look at metropolitan counties first. Does population make any difference in poverty rate among metropolitan counties? If so, is it necessary to divide "Metropolitan_S"(in area__ubif_sim) into "Metro_2" and "Metro_3"(in area__rucc_sim)?
```{r, echo=FALSE}
#for metropolitan from area__urbif_sim classification

temp <-TrainingData %>% filter(area__ubif_sim %in% c("Metropolitan_L", "Metropolitan_S")) %>% group_by(area__ubif_sim) %>% summarize(count=n(), median=round(median(poverty_rate),3), mean=round(mean(poverty_rate),3), var=round(var(poverty_rate),3))
...
#for metropolitan from area__rucc_sim classification

temp <- TrainingData %>% filter(area__rucc_sim %in% c("Metro_1", "Metro_2", "Metro_3")) %>% group_by(area__rucc_sim) %>% summarize(count=n(), median=round(median(poverty_rate),3), mean=round(mean(poverty_rate),3), var=round(var(poverty_rate),3))
...
To answer the second question regarding population of metro area, let's do another hypothesis test by using permutation. The result is shown below.
```{r, echo=FALSE}
M2M3 <- TrainingData %>% 
...
```
As the previous one, the distribution looks like a bell-shaped curve, centered at zero. The blue vertical lines indicate the 2.5% and 97.5% quantile respectively. But this time, clearly, the red line lies outside of the 95% zone, which means a less than 5% p-value.So I'd better follow the 3-category classification from area__rucc, instead of the 2-category classification from area_urban_influence, for metropolitan counties.
...
temp <- temp %>% group_by(pop_size) %>% summarize(count=n(), median=round(median(poverty_rate),3), mean=round(mean(poverty_rate),3), var=round(var(poverty_rate),3))
...
From the result above, population is making a difference in poverty rate, and mainly between "M" and "S". So there is an option combining "L" and "M" into one category to decrease the dimension. 
...
kable(temp %>% group_by(Adjacency) %>% summarize(count=n(), median=round(median(poverty_rate),3), mean=round(mean(poverty_rate),3), var=round(var(poverty_rate),3)))

temp %>% ggplot(aes(x=Adjacency, y=poverty_rate)) + geom_boxplot() + coord_flip()

#Adjacency in population size L
temp <- TrainingData %>% filter( area__rucc_sim %in% c("Nonmetro_1","Nonmetro_2")) %>% mutate(Adjacency_L = factor(ifelse(area__rucc_sim %in% c("Nonmetro_1"), "Y", "N"))) %>% select("area__rucc_sim","Adjacency_L", "poverty_rate") 

kable(temp %>% group_by(Adjacency_L) %>% summarize(count=n(), median=round(median(poverty_rate),3), mean=round(mean(poverty_rate),3), var=round(var(poverty_rate),3)))

temp %>% ggplot(aes(x=Adjacency_L, y=poverty_rate)) + geom_boxplot() + coord_flip()

#Adjacency in population size M

temp <- TrainingData %>% filter( area__rucc_sim %in% c("Nonmetro_3","Nonmetro_4")) %>% mutate(Adjacency_M = factor(ifelse(area__rucc_sim %in% c("Nonmetro_3"), "Y", "N"))) %>% select("area__rucc_sim","Adjacency_M", "poverty_rate") 

kable(temp %>% group_by(Adjacency_M) %>% summarize(count=n(), median=round(median(poverty_rate),3), mean=round(mean(poverty_rate),3), var=round(var(poverty_rate),3)))

temp %>% ggplot(aes(x=Adjacency_M, y=poverty_rate)) + geom_boxplot() + coord_flip()

#Adjacency in population size S

temp <- TrainingData %>% filter( area__rucc_sim %in% c("Nonmetro_5","Nonmetro_6")) %>% mutate(Adjacency_S = factor(ifelse(area__rucc_sim %in% c("Nonmetro_5"), "Y", "N"))) %>% select("area__rucc_sim","Adjacency_S", "poverty_rate") 

kable(temp %>% group_by(Adjacency_S) %>% summarize(count=n(), median=round(median(poverty_rate),3), mean=round(mean(poverty_rate),3), var=round(var(poverty_rate),3)))

temp %>% ggplot(aes(x=Adjacency_S, y=poverty_rate)) + geom_boxplot() + coord_flip()
```
Adjacency has the strongest effect in non-metro county of small population size, but for those of population size medium and large, the effect is not so obvious. What's counter intuitive is that among non-metro county, those adjacent to metro area tend to have higher poverty rate than those aren't. And this tendency grows as population size decreases.

(8)A brief look at "econ__economic_typology", then simplify and reorder it.
```{r, echo=FALSE}
#descriptive statistics
kable(TrainingData %>% group_by(econ__economic_typology) %>% summarize(count=n(), median=round(median(poverty_rate),3), mean=round(mean(poverty_rate),3), var=round(var(poverty_rate),3)))
